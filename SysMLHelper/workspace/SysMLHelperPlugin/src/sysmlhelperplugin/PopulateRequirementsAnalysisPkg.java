...
public class PopulateRequirementsAnalysisPkg {
	...
	public static void copyGatewayFilesIfPresent(IRPProject forProject){
		...
		if (theProfile != null){
			...
			IRPUnit theUnit = (IRPUnit)theProfile;
			IRPApplication myApp = RhapsodyAppServer.getActiveRhapsodyApplication();
			String pathToSearch = myApp.getOMROOT() + theUnit.getCurrentDirectory().replace("$OMROOT", "");
	...
	public static void createRequirementsAnalysisPkg(IRPProject forProject){
		...
		if (ok) {
		    ...
		    if (response == JOptionPane.YES_OPTION) {
				...
				boolean theAnswer = askAQuestion(
	...
	static IRPPackage addRequirementsAnalysisPkg(IRPProject toTheProject){
		...
		m_rhpApplication.addToModel("$OMROOT\\Profiles\\SysMLHelper\\SysMLHelper_rpy\\RequirementsAnalysisPkg.sbs", 1);
	...
	private static void applySimpleMenuStereotype(IRPProject toTheProject) {
		...
		if (theEl != null && theEl instanceof IRPStereotype){
			...
			Logger.writeLine(toTheProject, "was changed to " + theName + ". Remove the «SimpleMenu» stereotype to return the 'Add New' menu.");
	...
	public static IRPProfile addProfileIfNotPresent(String theProfileName, IRPProject toTheProject){
		...
		if (theProfile==null){
			...
			IRPUnit theUnit = m_rhpApplication.addProfileToModel(theProfileName);
	...
	public static boolean askAQuestion(String q){
		int answer = javax.swing.JOptionPane.showConfirmDialog(null, q, 
				null, 
